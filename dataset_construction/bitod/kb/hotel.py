...
class HotelEngine(BaseEngine):
    def __init__(
        ...
        kb_file = os.path.join(kb_loc, 'hotel_kb.json')
        supported_api_call = 'hotels_en_US_search'
        super().__init__(kb_file, supported_api_call)

        data = self.kb_df.copy().to_dict('records')
        self.entry_objs = [Hotel(obj, avail_prob=avail_prob) for obj in data]
