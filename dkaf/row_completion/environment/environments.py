...
class Environment(object):
    ...
    def reset(self):
        ...
        collate_fn = self.vocab.collate_fn
    ...
    def step_train(self, actions):
        ...
        actions = [self.vocab.all_latent_entities[act] for act in actions]
    ...
    def step_infer(self, actions, track_all=False):
        ...
        for idx in track_idxs:
            ...
            self.track_samples[-1]['edge'] = actions[idx]
    ...
    def get_tracking_samples(self, do_print=False):
        ...
        for obj in self.track_samples:
            n1 = obj['orig_entities'][obj['source_idx']]
            n2_pred = obj['edge']
            rel_tar = self.vocab.all_relations[obj['relation']]
            reward = obj['reward']

            ret = {
                'did': obj['did'],
                'target': [n1, rel_tar, n2_pred],
                'sign': obj['sign']
            }
...
class MAPOEnvironment(Environment):
    def __init__(
        ...
        self.explored_trajectories = set()
        self.good_traj_data = defaultdict(lambda: [])
        self.good_trajectories = set()
    ...
    def process_batch(self, batch):
        all_actions = self.vocab.all_latent_entities
        ...
        for obj in batch:
            ...
            rewards = self.reward_fn(
                states,
                [self.vocab.all_latent_entities[act] for act in not_explored_actions],
                mode='train'
            )
        ...
        tactions = [self.vocab.all_latent_entities[act] for act in actions]
        trewards = self.reward_fn(batch, tactions, mode='train')
        ...
        for idx, obj in enumerate(batch):
            ...
            if len(self.good_traj_data[sign]) > 0:
                ...
                jdx = np.random.choice(len(probs), p=probs)
                good_sample = self.good_traj_data[sign][jdx]
    ...
    def get_buffer_dataloader(self):
        ...
        print(Counter([x['reward'] for y in self.training_batches for x in y]))
