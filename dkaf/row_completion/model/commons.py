...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        """
        Args:
            x: Tensor, shape [batch_size, max_seq_len, embedding_dim]
        """
...
class FlatTransformerEncoder(nn.Module):
    def __init__(self, cfg):
        ...
        self.d_model = cfg['emb_size']  # 200
        n_head = cfg.get('n_head', 4)
        n_layers = cfg.get('n_layers', 2)
        d_hid = self.d_model * 2  # 400
        dropout = cfg.get('dropout', 0.0)

        # self.pos_embedding = PositionalEncoding(d_model=self.d_model, dropout=dropout, max_len=5000)
        encoder_layers = nn.TransformerEncoderLayer(self.d_model, n_head, d_hid, dropout=dropout, activation="gelu")
        self.transformer_encoder = nn.TransformerEncoder(encoder_layers, n_layers)

    def forward(self, token_features, token_lens):
        # features = self.pos_embedding(token_features)
        features = token_features
        features = features.permute(1, 0, 2)

        key_padding_mask = ~create_sequence_mask(token_lens, dtype=torch.bool)
        features = self.transformer_encoder(
            src=features, src_key_padding_mask=key_padding_mask
        )
