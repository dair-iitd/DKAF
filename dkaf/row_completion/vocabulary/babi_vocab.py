...
class Vocabulary(object):
    ...
    def prepare_samples(self, entry, src=None, rel=None, mode='infer'):
        ...
        kb = entry['kb']
        kb_entities = set([
            v for row in kb for v in row.values()
        ])

        kb_entity_types = dict()
        for row in kb:
        ...
        for idx in range(1, len(context), 2):
            ...
            if "api_call" in uttr:
                uttr_ents = toks[1:]
                kb_entity_types[uttr_ents[0]] = 'R_cuisine'
                kb_entity_types[uttr_ents[1]] = 'R_location'
                kb_entity_types[uttr_ents[2]] = 'R_number'
                kb_entity_types[uttr_ents[3]] = 'R_price'
            else:
                uttr_ents = list(kb_entities.intersection(set(toks)))

            for tok in uttr_ents:
                entities.append(tok)
                ent_uttr_indices.append(idx)
                ent_uttr_positions.append(toks.index(tok))
        ...
        for idx in range(len(entities)):
            ...
            tcontext[uttr_idx] = tcontext[uttr_idx].replace(ent, f"{self.mask}")
            ...
            ret['target_entity_type'] = kb_entity_types[ent]
            ret['sign'] = f"{entry['did']}_{ent}"
            ret['did'] = entry['did']
            ret['uttr_vocab_indices'] = tvocab_indices
