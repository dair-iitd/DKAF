...
class Vocabulary(object):
    ...
    def transform_entry(self, entry, mode='infer'):
        ...
        source_type, source_node, src_uttr_idx, src_uttr_pos = entry['ent1']
        toks = context[src_uttr_idx].split()
        assert toks[src_uttr_pos] == source_node
        toks[src_uttr_pos] = f"{self.E1S}###{source_node}###{self.E1E}"
        context[src_uttr_idx] = ' '.join(toks)
        toks = context_tag[src_uttr_idx].split()
        assert toks[src_uttr_pos] == source_type or source_node == 'dont_care'
        toks[src_uttr_pos] = f"null_tag###{source_type}###null_tag"
        context_tag[src_uttr_idx] = ' '.join(toks)

        target_type, target_node, tar_uttr_idx, tar_uttr_pos = entry['ent2']
        toks = context[tar_uttr_idx].split()
        assert toks[tar_uttr_pos] == target_node
        toks[tar_uttr_pos] = f"{self.E2S}###{target_node}###{self.E2E}"
        context[tar_uttr_idx] = ' '.join(toks)
        toks = context_tag[tar_uttr_idx].split()
        assert toks[tar_uttr_pos] == target_type or target_node == 'dont_care'
        toks[tar_uttr_pos] = f"null_tag###{target_type}###null_tag"
        context_tag[tar_uttr_idx] = ' '.join(toks)

        for ii in range(len(context)):
            toks = context[ii].split()
            toks = [tt for tok in toks for tt in tok.split("###")]
            context[ii] = ' '.join(toks)
            toks = context_tag[ii].split()
            toks = [tt for tok in toks for tt in tok.split("###")]
            context_tag[ii] = ' '.join(toks)
        ...
        assert all(
            ctx_ret['token_lens'][ii] == tag_ret['token_lens'][ii]
            for ii in range(len(context))
        )
        assert len(context) == len(context_tag)
